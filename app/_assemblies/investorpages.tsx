...
import { dossierFor, type DossierKey } from "./investordossier";
...
function InvestorWorkspace({ path, vehicle, param }: InvestorSurfaceProps) {
  ...
  return <main className="investor-surface"><header><Link href="/" className="sysmark" aria-label="Getaway Collective"><Wordmark size={14} /></Link><span>{preview ? "DESIGN PREVIEW / PLACEHOLDER MATERIAL" : `INVESTOR-CONFIDENTIAL / ${actualVehicle.toUpperCase()}`}</span></header><div className="investor-shell"><aside><span className="eyebrow">PRIVATE REVIEW</span><h2>One vehicle.<br />One record.</h2><nav>{tabs.map(([label, suffix]) => <Link key={label} className={key === (suffix ? suffix.slice(1) : "overview") ? "active" : ""} href={href(suffix)}>{label}</Link>)}</nav><Link className="investor-speak" href={href("/speak")}>Speak with IR →</Link></aside><section className="investor-main"><span className="eyebrow">{item.eyebrow}</span><h1>{dossier?.title ?? item.title}</h1><p className="investor-lead">{dossier?.lead ?? item.lead}</p>{dossier ? dossier.sections.map((sec) => <div className="investor-dossier" key={sec.heading}><span>{sec.heading.toUpperCase()}</span>{sec.rows.map((r) => <article key={r.label}><h3>{r.label}</h3><strong>{r.value}</strong><em>{r.basis}</em></article>)}</div>) : <div className="investor-proof"><span>WHAT SUPPORTS THIS VIEW</span>{item.evidence.map((e, i) => <article key={e}><b>0{i + 1}</b><p>{e}</p><em>Verified record →</em></article>)}</div>}<div className="investor-action"><p>{dossier?.note ?? item.note}</p><Link className="btn primary" href={dossier?.actionHref ?? href(key === "overview" ? "/asset" : key === "asset" ? "/financials" : key === "financials" ? "/structure" : key === "structure" ? "/risks" : key === "risks" ? "/dataroom" : key === "dataroom" ? "/commit" : "/speak")}>{dossier?.action ?? item.action}</Link></div></section></div></main>;
}
