...
open_rows = [
    ['BLANK-23', 'L1-01 §23 vs L1-14',
     'FINANCIAL invariant ID collision. L1-01 §23 assigns F-03 = Governance Determinism and F-04 = Waterfall Determinism. L1-14 assigns F-03 = Distributions Follow Waterfall and F-04 = Reserve Floor Enforced. Two documents give the same IDs to different rules. Violates §14 Naming Authority.',
     'PROPOSED: adopt the L1-14 numbering as canonical (it is the dedicated invariant register) and renumber L1-01 §23. Also fold in the 4 new invariants from L1-16 (F-10 Revenue Base Determinism, F-11 Capital Call Purpose Gate, F-12 Reserve Non-Pooling, F-13 Brand Participation Is Not Equity). Net result: one FINANCIAL invariant table, 13 entries. MUST be fixed before any F-invariant reaches code.',
     'L5, L10, all F-invariants', 'Enterprise Architecture', 'HIGH', ''],
    ['BLANK-09a', 'L1-16 §2.6',
     'Reserve breach response - does the distribution suspension survive? Instruction was "just a broadcast is sent and expenses are delayed". The BLANK-09 ruling had explicitly said discretionary distributions ARE suspended. The later instruction did not say to remove it.',
     'IMPLEMENTED CONSERVATIVELY: suspension RETAINED alongside broadcast + expenditure deferral. If distributions should continue during a breach, say so - but note that partner distributions continuing while the reserve is below floor materially weakens invariant F-04. One-line confirmation closes this.',
     'F-04, L5', 'Finance', 'MEDIUM', ''],
]
for r in open_rows:
    ws.append(r)
...
for line in [
    ...
    [''],
    ['OPEN: 2   (BLANK-23 F-invariant ID collision, BLANK-09a distribution-suspension confirmation)'],
    ['RATIFIED: 26'],
    [''],
